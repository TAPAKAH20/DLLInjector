...
DWORD FindLoadLibrary(HANDLE hProc, HANDLE hThread, ULONG_PTR& loadLibAddr) {
	...
	for (DWORD i = 0; i < amount; i++) {
		...
		IF_FAIL_RET(eStatus, ReadRemote<IMAGE_NT_HEADERS>(hProc, moduleBase + imageDosHeader.e_lfanew, imageNtHeader));
		...
		if (strcmp(moduleName, targetLib) == 0) {
			...
			for (DWORD j = 0; j < numOfFuncs; j++) {
				...
				if (strcmp(functionName, targetFunc) == 0) {
					loadLibAddr = functionAddrsRVA[j];
					break;
				}
			}
...
int main(int argc, char* const argv[]) 
{
	...
	//Inject
	IF_FAIL_GO(status, Inject(hProc, hThread, loadLibAddr), MAIN_ERROR_HANDLER);
